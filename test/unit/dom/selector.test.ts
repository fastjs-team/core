...
test("Select div with selector 'div'", () => {
  ...
  assert(el?.el() === document.body.querySelector("div"), "first div should be selected");
});

test("Select div with selector 'div#root'", () => {
  const el = dom.select("div#root");
  assert(el?.el() === document.body.querySelector("div#root"), "root div should be selected");
})

test("Select div with selector 'div.div-child'", () => {
  const el = dom.select("div.div-child");
  assert(el?.el() === document.body.querySelector("div.div-child"), "first div with class div-child should be selected");
});

test("Select div with selector 'div.div-child' and index 1", () => {
  const el = dom.select("div.div-child");
  assert(el?.getDom(1).el() === document.body.querySelectorAll("div.div-child")[1], "second div with class div-child should be selected");
})
